...
class TestGenreHydrationAndMediaUrl:
    ...
    def test_create_or_get_from_volume_hydrates_categories(self):
        ...
        assert any(cat in cat_names for cat in ['Fiction', 'Ficción'])
        assert any(cat in cat_names for cat in ['Thrillers', 'Thriller y Suspense'])
        assert any(cat in cat_names for cat in ['Suspense', 'Thriller y Suspense'])
        assert any(cat in cat_names for cat in ['Horror', 'Terror'])
