...
class GoogleBooksProvider:
    """Proveedor para consultar la API de Google Books v1 con soporte de caché Redis."""
    ...
    def search_by_title(self, title: str, offset: int = 0, limit: int = 8) -> list[ProviderBookData]:
        ...
        cache_key = google_search_key(clean_title, offset)
        try:
            cached_items = cache.get(cache_key)
            if cached_items is not None:
                return [self._parse_volume(item) for item in cached_items]
        except Exception as e:
            logger.warning(f"Error accediendo a caché para '{clean_title}': {e}")
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                try:
                    cache.set(cache_key, items, timeout=TTL_EXTERNAL_API // 4)
                except Exception as ce:
                    logger.warning(f"Error guardando caché de búsqueda Google Books: {ce}")
    ...
    def get_by_isbn(self, isbn: str) -> ProviderBookData | None:
        ...
        try:
            cached_vol = cache.get(cache_key)
            if cached_vol is not None:
                data = self._parse_volume(cached_vol)
                if not data.isbn:
                    data.isbn = isbn
                return data
        except Exception as e:
            logger.warning(f"Error accediendo a caché para ISBN {isbn}: {e}")
        ...
        try:
            ...
            if resp.ok:
                ...
                if items:
                    volume = items[0]
                    try:
                        cache.set(cache_key, volume, timeout=TTL_EXTERNAL_API)
                    except Exception as ce:
                        logger.warning(f"Error guardando en caché ISBN {isbn}: {ce}")

                    data = self._parse_volume(volume)
                    if not data.isbn:
                        data.isbn = isbn
                    return data
        except Exception as e:
